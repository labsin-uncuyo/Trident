...
# Modified: Use OpenAI instead of Ollama
...
def activity():
    """Generate activity suggestions for NPCs"""
    global docs
    global documents
...
def chat():
    """Generate chat content for NPCs"""
    logger.debug(f"Using OpenAI for chat with model {LLM_MODEL}")
...
def content():
    """Generate document content for NPCs"""
    global docs
    global documents
    response = ""
    
    while len(response) == 0:
        # Use OpenAI for all content types
        web_llm = ChatOpenAI(
            model=LLM_MODEL,
            temperature=0.9,
            openai_api_key=OPENAI_API_KEY,
...
def social():
    """Generate social media content for NPCs"""
    llm = ChatOpenAI(
        model=LLM_MODEL,
        temperature=0.9,
        openai_api_key=OPENAI_API_KEY,
...
def lessons():
    """Generate training/lesson content"""
    global docs
    global documents
    data = request.json
    query = data.get('query')
...
def health():
    ...
    return jsonify({
        "status": "healthy",
        "service": "ghosts-shadows-openai",
        "llm_provider": "openai",
        "model": LLM_MODEL
    })

if __name__ == '__main__':
    logger.info("Starting GHOSTS Shadows API with OpenAI support")
    logger.info(f"Model: {LLM_MODEL}")
    logger.info(f"Base URL: {OPENAI_BASE_URL}")
    api_app.run(host='0.0.0.0', port=5900, debug=False)
