...
# Planner API
# Redirect planner stderr to a dedicated log file for debugging
mkdir -p "/outputs/${RUN_ID}/logs"
python3 -m uvicorn defender.app.main:app --host 127.0.0.1 --port "${PLANNER_PORT:-1654}" --log-level info 2>"/outputs/${RUN_ID}/logs/planner_api_stderr.log" &
PLANNER_PID=$!
