...
class SetEvidenceHelper:
    ...
    def password_guessing(self, twid, flow, uids: List[str], attempts: int) -> None:
        """
        Detects HTTP password guessing attempts to login endpoints.
        ...
        # Determine the port (80 or 443 typically)
        dst_port = getattr(flow, 'dport', 'unknown')

        description: str = (
            f"HTTP password guessing detected. "
            f"Src IP {flow.saddr} made {attempts} login attempts to "
            f"{flow.host}{flow.uri} on port {dst_port}. "
            f"Detected by Slips."
        )
